...
void takeAction( std::istream& is, std::ostream& os, const std::string& choice, PPM& input_image1, PPM& input_image2, PPM& output_image){
	if (choice == "write"){
		...
		plusEqualsInt(is,os,input_image1);
		return;
	}
	else if (choice == "-="){
...
void plusEqualsInt(std::istream& is, std::ostream& os, PPM& src){
	int boost = getInteger(is,os,"Boost? ");
	src += boost;
	return;
}
