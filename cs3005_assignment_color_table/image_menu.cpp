...
int imageMenu(std::istream& is, std::ostream& os){
	...
	int run = 0;
	std::string choice;
	ColorTable *color = new ColorTable(16);
	size_t tempone = 0;
	size_t temptwo = 15;
	Color *d1 = new Color(0,255,0);
	Color *d2 = new Color(255,0,255);
	color -> insertGradient(*d1,*d2,tempone,temptwo);
	while (run == 0){
		if (choice == "quiet"){
			quiet = not quiet;
		}
		if (choice == "quit"){
			...
			takeAction(is,cloak,choice,input_image1,input_image2,output_image,*gptr,*color);
			if (choice == "julia"){
			...
			takeAction(is,os,choice,input_image1,input_image2,output_image,*gptr,*color);
			if (choice == "julia"){
                	        setJuliaFractal(is,os,gptr);
                }
	                if (choice == "mandelbrot"){
...
int runFile(std::istream& is, std::ostream& os, PPM& input_image1, PPM& input_image2, PPM& output_image, NumberGrid& grid, ColorTable& table){
	...
	while(file.good() and choice != "quit"){
		...
		takeAction(file,flow,choice,input_image1,input_image2,output_image,grid,table);
	}
	return 0;
}
...
void setColorGradient(std::istream& is, std::ostream& os, ColorTable& table){
	...
	p1 = getInteger(is,os, "First position? ");
        r1 = getInteger(is,os,"First red? ");
        g1 = getInteger(is,os,"First green? ");
        b1 = getInteger(is,os,"First blue? ");
	p2 = getInteger(is,os, "Second position? ");
        r2 = getInteger(is,os,"Second red? ");
        g2 = getInteger(is,os,"Second green? ");
        b2 = getInteger(is,os,"Second blue? ");
	Color c1(r1,g1,b1);
	Color c2(r2,g2,b2);
	table.insertGradient(c1,c2,p1,p2);
	return;
}
