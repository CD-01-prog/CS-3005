...
void NumberGrid::calculateAllNumbers(){
	...
	for(;i <= mheight; ++i){
		for(j=0; j<=mwidth;++j){
			value_holder = calculateNumber(i,j);
			setNumber(i,j,value_holder);
		}
	}
	return;
}
...
void NumberGrid::setPPM(PPM& ppm, const ColorTable& colors) const{
	...
	for(; i <= mheight; ++i){
			...
			c = getNumber(i,j);
			if ( c >= mMaxValue){
				ppm.setPixel(i,j,colors[num-1].getRed(),colors[num-1].getGreen(),colors[num-1].getBlue());

			}
			else if (c == 0){
				ppm.setPixel(i,j,colors[num-2].getRed(),colors[num-2].getGreen(),colors[num-2].getBlue());
			}
			else{
				ppm.setPixel(i,j,colors[c%(num-2)].getRed(),colors[c%(num-2)].getGreen(),colors[c%(num-2)].getBlue());
				}
